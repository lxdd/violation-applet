...
import com.auto.applet.demo.dao.MyEventRepository;
...
/**
 * 让数据在Http上双向无限流动起来
 * Websocket确实可以实现全双工通信，但它的数据传输并非是完全基于HTTP协议的
 * @author li_xiaodong
 * @date 2018年6月1日
 */
@RestController
...
public class MyEventController {
	...
	/**
	 * POST方法的接收数据流的Endpoint，所以传入的参数是一个Flux，返回结果其实就看需要了，我们用一个Mono<Void>作为方法返回值，
	 * 表示如果传输完的话只给一个“完成信号”就OK了；
	 * 
	...
	/**
	 * GET方法的无限发出数据流的Endpoint，所以返回结果是一个Flux<MyEvent>，不要忘了注解上produces =
